...
class ReporterSignupFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tilFullName.editText?.addTextChangedListener(createWatcher { text -> viewModel.fullName.value = text })
        binding.tilEmail.editText?.addTextChangedListener(createWatcher { text -> viewModel.email.value = text })
        binding.tilPhone.editText?.addTextChangedListener(createWatcher { text -> viewModel.phone.value = text })
        binding.tilPassword.editText?.addTextChangedListener(createWatcher { text -> viewModel.password.value = text })
        binding.tilConfirmPassword.editText?.addTextChangedListener(createWatcher { text ->
            // You can add a property for confirm password in the ViewModel if needed
        })
