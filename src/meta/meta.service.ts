...
export class MetaService {
    ...
    async getEnums(){
         ...
         */
        return Object.entries(enums).reduce((acc, [key, enumObj])=>{
            acc[key] = Object.values(enumObj);
            return acc;
        }, {} as Record<string, string[]>);// type assertion to prevent typescript stricte erros
    }
    ...
    async getLookupTables(){
        ...
        await Promise.all([
            ...
            this.prisma.vulnerabilityStatus.findMany({ select: { id: true, name: true}})
        ]);

        return {
            sectors, caseTypes, serviceTypes, vulnerabilityStatuses,
        }
